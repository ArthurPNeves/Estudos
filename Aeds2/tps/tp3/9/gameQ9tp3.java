...
class lista {
    private gameQ9tp3[] ArrayLista = new gameQ9tp3[100];
    private int n = 0;
    private int comparacoes = 0;
    private int movimentacoes = 0;
    private long tempofinal = 0;

    public gameQ9tp3[] getArrayLista() {
        return ArrayLista;
    }

    public void setArrayLista(gameQ9tp3[] arrayLista) {
    ...
    public lista(gameQ9tp3[] arrayLista, int n) {
        ArrayLista = arrayLista;
        this.n = n;
    }

    void inserirInicio(gameQ9tp3 GAME) throws Exception {
    ...
    void inserir(gameQ9tp3 GAME, int posicao) throws Exception {
    ...
    void inserirFim(gameQ9tp3 GAME) throws Exception {
        if (n >= ArrayLista.length) {
    ...
    gameQ9tp3 removerInicio() throws Exception {
        if (n == 0) {
            throw new Exception("Erro removerInicio");
        }
        gameQ9tp3 GAME = ArrayLista[0];
    ...
    gameQ9tp3 remover(int posicao) throws Exception {
        if (n == 0 || posicao < 0 || posicao >= n) {
            throw new Exception("Erro remover");
        }

        gameQ9tp3 GAME = ArrayLista[posicao];
        n--;
    ...
    gameQ9tp3 removerFinal() throws Exception {
        if (n == 0) {
    ...
    gameQ9tp3[] ordernarLista(int start, int end)
    {
        gameQ9tp3[] left, right, aux = new gameQ9tp3[end-start+1];
        int i = 0, j = 0, k = 0;

        if (start < end) {
            left = ordernarLista( start, (end+start)/2);
            right = ordernarLista( (start+end)/2+1, end);

        
                while (j < right.length && k < left.length) {
                    if ( left[k].getUpvotes() >= right[j].getUpvotes()  ) 
                        aux[i++] = right[j++];
                    else 
                        aux[i++] = left[k++];
                }
                while (j < right.length) aux[i++] = right[j++];
                while (k < left.length) aux[i++] = left[k++];

        } else {
            aux[0] = ArrayLista[start];
        }
    ...
    gameQ9tp3 lerId(int id) throws Exception {
        gameQ9tp3 GAME = new gameQ9tp3();
        String aux = "";
        int j = 0;
        String aux2 = "1";
    ...
    void escreverLog() throws IOException {
        ...
        BufferedWriter arquivo = new BufferedWriter(new FileWriter("matrícula_ordernarLista.txt"));
...
public class gameQ9tp3 {
    ...
    public gameQ9tp3(int app_id, String name, Data release_date, String owners, int age, float price, int dlcs,
    ...
    public static void main(String[] args) throws Exception {
        ...
        LISTA.setArrayLista(LISTA.ordernarLista(0, (LISTA.getN() - 1)));
        long Final = System.currentTimeMillis();
        LISTA.setTempofinal(Final - inicio);
        LISTA.imprimirLista();
        LISTA.escreverLog();
    }
