...
class lista {
    private gameQ1tp3[] ArrayLista = new gameQ1tp3[75];
    private int n = 0;
    private int comparacoes = 0;
    private int movimentacoes = 0;
    private long tempofinal = 0;

    public gameQ1tp3[] getArrayLista() {
        return ArrayLista;
    }

    public void setArrayLista(gameQ1tp3[] arrayLista) {
    ...
    public lista(gameQ1tp3[] arrayLista, int n) {
        ArrayLista = arrayLista;
        this.n = n;
    }

    void inserirInicio(gameQ1tp3 GAME) throws Exception {
    ...
    void inserir(gameQ1tp3 GAME, int posicao) throws Exception {
    ...
    void inserirFim(gameQ1tp3 GAME) throws Exception {
        if (n >= ArrayLista.length) {
    ...
    gameQ1tp3 removerInicio() throws Exception {
        if (n == 0) {
            throw new Exception("Erro removerInicio");
        }
        gameQ1tp3 GAME = ArrayLista[0];
    ...
    gameQ1tp3 remover(int posicao) throws Exception {
        if (n == 0 || posicao < 0 || posicao >= n) {
            throw new Exception("Erro remover");
        }

        gameQ1tp3 GAME = ArrayLista[posicao];
        n--;
    ...
    gameQ1tp3 removerFinal() throws Exception {
        if (n == 0) {
    ...
    void ordernarLista()
    {
        ...
        for (int i = 0; i < (n - 1); i++) {
            int menor = i;
             for (int j = (i + 1); j < n; j++){
                comparacoes++;
             if (ArrayLista[menor].getName().compareTo( ArrayLista[j].getName()) >= 0 ){
             menor = j;
             }
             }
             gameQ1tp3 temp = ArrayLista[menor];
             ArrayLista[menor] = ArrayLista[i];
             ArrayLista[i] = temp;
    ...
    gameQ1tp3 lerId(int id) throws Exception
    {
        gameQ1tp3 GAME = new gameQ1tp3();
        String aux = "";
        int j = 0;
        String aux2 = "1";
     ...
     void escreverLog() throws IOException {
        ...
        BufferedWriter arquivo = new BufferedWriter(new FileWriter("matrícula_selecao.txt"));
...
public class gameQ1tp3 {
    ...
    public gameQ1tp3(int app_id, String name, Data release_date, String owners, int age, float price, int dlcs,
