...
class lista {
    private gameQ6tp3[] ArrayLista = new gameQ6tp3[75];
    private int n = 0;
    private int comparacoes = 0;
    private int movimentacoes = 0;
    private long tempofinal = 0;

    public gameQ6tp3[] getArrayLista() {
        return ArrayLista;
    }

    public void setArrayLista(gameQ6tp3[] arrayLista) {
    ...
    public lista(gameQ6tp3[] arrayLista, int n) {
        ArrayLista = arrayLista;
        this.n = n;
    }

    void inserirInicio(gameQ6tp3 GAME) throws Exception {
    ...
    void inserir(gameQ6tp3 GAME, int posicao) throws Exception {
    ...
    void inserirFim(gameQ6tp3 GAME) throws Exception {
        if (n >= ArrayLista.length) {
    ...
    gameQ6tp3 removerInicio() throws Exception {
        if (n == 0) {
            throw new Exception("Erro removerInicio");
        }
        gameQ6tp3 GAME = ArrayLista[0];
    ...
    gameQ6tp3 remover(int posicao) throws Exception {
        if (n == 0 || posicao < 0 || posicao >= n) {
            throw new Exception("Erro remover");
        }

        gameQ6tp3 GAME = ArrayLista[posicao];
        n--;
    ...
    gameQ6tp3 removerFinal() throws Exception {
        if (n == 0) {
    ...
    {
        int i = esq, j = dir;
        gameQ6tp3 pivo = ArrayLista[(dir+esq)/2];
        while (i <= j) {
            while (compararDate(pivo, ArrayLista[i])) 
            {i++; comparacoes++;}
            
            while (compararDate( ArrayLista[j], pivo)) 
            {j--; comparacoes++;}
            if (i <= j) {
                swap(i, j);
                movimentacoes++;
     ...
     {
        gameQ6tp3 temp = ArrayLista[x];
        ArrayLista[x] = ArrayLista[y];
        ArrayLista[y] = temp;
      }
     
      public boolean compararDate(gameQ6tp3 primeiro, gameQ6tp3 segundo)
     {
        int mesEmNum1 = 0;
        int mesEmNum2 = 0;
    ...
    gameQ6tp3 lerId(int id) throws Exception {
        gameQ6tp3 GAME = new gameQ6tp3();
        String aux = "";
        int j = 0;
        String aux2 = "1";
    ...
    void escreverLog() throws IOException {
        ...
        BufferedWriter arquivo = new BufferedWriter(new FileWriter("matrícula_quicksort.txt"));
...
public class gameQ6tp3 {
    ...
    public gameQ6tp3(int app_id, String name, Data release_date, String owners, int age, float price, int dlcs,
    ...
    public static void main(String[] args) throws Exception {
        ...
        LISTA.ordernarLista(0, LISTA.getN() - 1);
