...
bool MAIOR(Game left, Game right, int *comparacoes)
{
    *comparacoes += 1;
...
void insertColour(int cor, int h, Game game[], int n_games, int *comparacoes, int *movimentacoes)
{
    for (int i = (h + cor); i < n_games; i += h)
    {
        *movimentacoes += 1;
        Game tmp = game[i];
        int j = i - h;
        while ((j >= 0) && MAIOR(game[j], tmp, comparacoes))
        {
            *movimentacoes += 1;
            game[j + h] = game[j];
            j -= h;
...
void shellsort(Game game[], int n_games, int *comparacoes, int *movimentacoes)
{
    ...
    do
    {
        ...
        {
            insertColour(cor, h, game, n_games, comparacoes, movimentacoes);
        }
    } while (h != 1);
}
...
int MAIORDois(Game j, Game temp, int *comparacoes)
{
    *comparacoes += 1;
    int result = 0;
    if (FloatRound(j.upvotes * 1000) > FloatRound(temp.upvotes * 1000))
    {
        result = 1;
    }
    else
...
void countingsort(Game game[], int n, int *comparacoes, int *movimentacoes)
{
    ...
    for (int i = 0; i < n; i++)
    {
        tmp = FloatRound(game[i].upvotes * 1000);
        count[tmp]++;
        *movimentacoes += 1;
    }
    ...
    for (int i = 1; i < n; i++)
    {
        *movimentacoes += 1;
        ...
        int j = i - 1;
        while ((j >= 0) && MAIORDois(ordenado[j], temporario, comparacoes))
        {
            *movimentacoes += 1;
