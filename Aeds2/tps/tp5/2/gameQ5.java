...
class Data {
 ...
 class No1 {
    ...
    void fazerAcoes(String s1) throws Exception {
    ...
    public void inserir(gameQ5 game) {
        raizNo = inserir(raizNo, game);
    }

    public No inserir(No i, gameQ5 game) {
        if (i == null) {
            i = new No(game, null, null);
        } else if (game.getName().compareTo(i.getObj().getName()) > 0) {
            i.setDir(inserir(i.getDir(), game));
        } else if (game.getName().compareTo(i.getObj().getName()) < 0) {
            i.setEsq(inserir(i.getEsq(), game));
        } else {
    ...
    public No1(char letra, No1 esq, No1 dir, No raizNo) {
        this.letra = letra;
        this.esq = esq;
        this.dir = dir;
        this.raizNo = raizNo;
    ...
    public void setRaizNo(No raizNo) {
        this.raizNo = raizNo;
    }
...
class No {
    ...
    public No(gameQ5 obj, No esq, No dir) {
        this.obj = obj;
        this.esq = esq;
        this.dir = dir;
    }
...
class Arvore {
    ...
    public No1 inserir(No1 i, char c) {
        if (i == null) {
            i = new No1(c, null, null, null);
        } else if (c > i.getLetra()) {
            i.setDir(inserir(i.getDir(), c));
        } else if (c < i.getLetra()) {
            i.setEsq(inserir(i.getEsq(), c));
        } else {
            System.out.println("erro");
        }
        return i;
    ...
    public Boolean caminharPre(No1 i, String nome)
    {   boolean result = false;
        if (i != null) {
           result = i.pesquisar(nome);
           
            caminharPre(i.getEsq(), nome);
            System.out.print("ESQ ");
            caminharPre(i.getDir(), nome);
            System.out.print("DIR ");
        }
        return result;
    }
    ...
    public No1 pesquisarNoDeLetra(String s1) throws NumberFormatException, Exception{
        No1 tmp = raiz;
        char c;
        if (s1.charAt(0) == 'I' && s1.charAt(1) == ' ' ) {
            String s2 = "";
                for (int j = 2; j < s1.length(); j++) {
                    s2 = s2 + s1.charAt(j);
                }
                c = lerId(Integer.parseInt(s2)).getName().charAt(0);
        } else if(s1.charAt(0) == 'R' && s1.charAt(1) == ' ') {
            String s2 = "";
            for (int j = 2; j < s1.length(); j++) {
                s2 = s2 + s1.charAt(j);
            }
        ...
        while (tmp != null && tmp.getLetra() != c) {
            if (c > tmp.getLetra()) {
               
                tmp = tmp.getDir();
            } else if (c < tmp.getLetra()) {
    ...
    public void setRaiz(No1 raiz) {
        this.raiz = raiz;
    }
    public Arvore(No1 raiz) {
        this.raiz = raiz;
    }
    public Arvore() {
    }
...
public class gameQ5 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (isFim(id) == false) {
            ...
            ARV.pesquisarNoDeLetra(GAME.getName()).inserir(GAME);
        ...
        for (int i = 0; i < n; i++) {
            s1 = MyIO.readLine();
            ARV.pesquisarNoDeLetra(s1).fazerAcoes(s1);

        }
        String s2;
        s2 = MyIO.readLine();
        while (isFim(s2) == false) {
            ARV.caminharPre(s2);
            s2 = MyIO.readLine();
        }
