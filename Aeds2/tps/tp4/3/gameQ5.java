...
class lista {
    ...
    void inserir(gameQ5 GAME) throws Exception {
        if (tamanho == 5) {
            remover();
    ...
    void inserirAntesFim(gameQ5 GAME) throws Exception {
    ...
    gameQ5 remover() throws Exception {
        Celula tmp = primeiro;
        primeiro = primeiro.getProx();
        gameQ5 obj = tmp.getObj();
        tmp.setProx(null);
        tmp = null;
        tamanho--;
        return obj;
    ...
    void fazerMediaAvg() {
        ...
        for (; tmp != null;) {
            ...
            media = media + tmp.getObj().getAvg_pt();
            tmp = tmp.getProx();
        }

        parteinteira = (int) media / tamanho;
        if (media / tamanho - parteinteira >= 0.5) {
            media_avgpt = (((int) media) /tamanho) + 1;
        } else {
            media_avgpt = (((int) media) / tamanho);
        }

        System.out.println(media_avgpt);
    }
