...
// Functions - List
void lista_inserirInicio(Game x) {

    if(n >= MAXIMO_JOGOS) {

        printf("Insert error: MAXIMO_JOGOS reached");
        exit(1);
    } 
...
void lista_inserir(Game x, int pos) {

    if(n >= MAXIMO_JOGOS || (pos < 0 || pos > n)) {

        printf("Insert error: %s", n >= MAXIMO_JOGOS ? "MAXIMO_JOGOS reached" : "Invalid position");
        exit(1);
    }
...
int main() {
    ...
    while(true) {
        ...
        if(found.app_id != -1) lista_inserirFim(found);
    ...
    for(int i = 0; i < n_ops; i++) {
        ...
        if(line_in[0] == 'I') {
            ...
            if(line_in[1] == 'I') lista_inserirInicio(game_search(atoi(params)));
            else if(line_in[1] == 'F') lista_inserirFim(game_search(atoi(params)));
            else if(line_in[1] == '*') {
                ...
                lista_inserir(game_search(atoi(appId)), atoi(pos));
            }
        }
        else if(line_in[0] == 'R') {
